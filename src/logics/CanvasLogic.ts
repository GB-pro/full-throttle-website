...
// ★追加: ブリンク判定用のヘルパー関数
function isBlinking(time: number, offset: number): string | null {
  // timeとoffsetを使ってサイン波を作り、その頂点付近だけtrueにする
  // 頻度と持続時間はここで調整
  const trigger = Math.sin(time * 0.01 + offset); 
  if (trigger > 0.98) { // 閾値を高くすることで「一瞬」を表現
    // 水色か黄色をランダムに返す
    return Math.random() > 0.5 ? "#00f3ff" : "#ffee00";
  }
  return null;
}
...
        
        // ★追加2: ブリンク判定 (文字ごとにタイミングをずらす)
        const blinkColor = isBlinking(time, d.index * 100);
        ctx.fillStyle = blinkColor || "#000000"; 
        ctx.fillText(d.char, 0, 0);
        
        // ★追加1: 外枠を太く統一 (0.02 -> 0.05)
        ctx.strokeStyle = "#ffffff";
        ctx.lineWidth = Math.max(3, d.size * 0.05);
...
      ctx.textAlign = "center";
      ...
      // ★追加2: ブリンク判定 (擬音全体で判定)
      const pBlinkColor = isBlinking(time, 50);
      ctx.fillStyle = pBlinkColor || "#000000";
      
      // ★追加1: 外枠を太く統一 (0.02 -> 0.05)
      ctx.strokeStyle = "#ffffff";
      ctx.lineWidth = Math.max(3, pSize * 0.05);
